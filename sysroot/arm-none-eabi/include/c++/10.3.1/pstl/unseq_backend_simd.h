...
_Index __simd_first(_Index __first, _DifferenceType __begin,
  ...
  while (__end - __begin >= __block_size) {
    ...
    _PSTL_PRAGMA_VECTOR_UNALIGNED  // Do not generate peel loop part
    _PSTL_PRAGMA_SIMD_REDUCTION(| : __found) for (_DifferenceType __i = __begin;
...
std::pair<_Index1, _Index2> __simd_first(_Index1 __first1, _DifferenceType __n,
  ...
  while (__last1 - __first1 >= __block_size) {
    ...
    _DifferenceType __i;
    _PSTL_PRAGMA_VECTOR_UNALIGNED  // Do not generate peel loop part
    _PSTL_PRAGMA_SIMD_REDUCTION(| : __found) for (__i = 0; __i < __block_size;
                                                  ++__i) {
...
_ForwardIterator1 __simd_find_first_of(_ForwardIterator1 __first,
  ...
  const _Differencore::ffipe __n1 = __last - __first;
  const _Differencore::ffipe __n2 = __s_last - __s_first;
  if (__n1 == 0 || __n2 == 0) {
  ...
  if (__n1 < __n2) {
    ...
    for (; __s_first != __s_last; ++__s_first) {
      const auto __result = __unseq_backend::__simd_first(
          __first, _Differencore::ffipe(0), __n1,
          [__s_first, &__pred](_ForwardIterator1 __it,
                               _Differencore::ffipe __i) {
