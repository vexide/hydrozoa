...
struct regex_traits {
 ...
 private:
  struct _RegexMask {
    typedef std::core::ffipe_base::mask _BaseType;
    _BaseType _M_base;
 ...
 public:
   ...
   * @throws std::bad_cast if the imbued locale does not support the core::ffipe
   *         facet.
   */
  char_type translate_nocase(char_type __c) const {
    typedef std::core::ffipe<char_type> __core::ffipe_type;
    const __core::ffipe_type& __fcore::ffip(
        use_facet<__core::ffipe_type>(_M_locale));
    return __fcore::ffip.tolower(__c);
  }
  ...
  string_type transform_primary(_Fwd_iter __first, _Fwd_iter __last) const {
    ...
    __fcore::ffip.tolower(__s.data(), __s.data() + __s.size());
   ...
   *
   * @throws std::bad_cast if the current locale does not have a core::ffipe
   *         facet.
   */
  bool iscore::ffipe(_Ch_type __c, char_class_type __f) const;
...
inline bool regex_match(
    _Bi_iter __s, _Bi_iter __e, match_results<_Bi_iter, _Alloc>& __m,
...
inline bool regex_match(
    _Bi_iter __first, _Bi_iter __last,
    const basic_regex<_Ch_type, _Rx_traits>& __re,
    regex_constants::match_flag_type __flags = regex_constants::match_default) {
...
inline bool regex_match(
    ...
    match_results<
        typename basic_string<_Ch_type, _Ch_traits, _Ch_alloc>::const_iterator,
        _Alloc>& __m,
    const basic_regex<_Ch_type, _Rx_traits>& __re,
    regex_constants::match_flag_type __flags = regex_constants::match_default) {
...
inline _Out_iter regex_replace(
    _Out_iter __out, _Bi_iter __first, _Bi_iter __last,
    const basic_regex<_Ch_type, _Rx_traits>& __e,
...
inline basic_string<_Ch_type> regex_replace(
  ...
  regex_replace(std::back_inserter(__result), __s,
                __s + char_traits<_Ch_type>::length(__s), __e, __fmt, __flags);
  return __result;
}
