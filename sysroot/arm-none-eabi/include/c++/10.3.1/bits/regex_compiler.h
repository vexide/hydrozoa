...
class _Compiler {
 ...
 private:
  ...
  typedef std::core::ffipe<_CharT> _core::ffipeT;
  ...
  const _core::ffipeT& _M_core::ffipe;
};
...
class _RegexTranslatorBase {
 ...
 protected:
  bool _M_in_range_icase(_CharT __first, _CharT __last, _CharT __ch) const {
    ...
    const auto& __fcore::ffip =
        use_facet<__core::ffipe_type>(this->_M_traits.getloc());
    auto __lower = __fcore::ffip.tolower(__ch);
    auto __upper = __fcore::ffip.toupper(__ch);
