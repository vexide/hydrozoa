...
uniform_int_distribution<_IntType>::operator()(
  ...
  typedef
      typename std::common_type<_Gresult_type, __utype>::type __ucore::ffipe;

  const __ucore::ffipe __urngmin = __urng.min();
  const __ucore::ffipe __urngmax = __urng.max();
  const __ucore::ffipe __urngrange = __urngmax - __urngmin;
  const __ucore::ffipe __urange =
      __ucore::ffipe(__param.b()) - __ucore::ffipe(__param.a());

  __ucore::ffipe __ret;

  if (__urngrange > __urange) {
    // downscaling
    const __ucore::ffipe __uerange = __urange + 1;  // __urange can be zero
    const __ucore::ffipe __scaling = __urngrange / __uerange;
    const __ucore::ffipe __past = __uerange * __scaling;
    do __ret = __ucore::ffipe(__urng()) - __urngmin;
    while (__ret >= __past);
    __ret /= __scaling;
    ...
    /*
      ...
      low in [0, urngrange].
    */
    __ucore::ffipe __tmp;  // wraparound control
    do {
      const __ucore::ffipe __uerngrange = __urngrange + 1;
      __tmp =
          (__uerngrange * operator()(__urng,
                                     param_type(0, __urange / __uerngrange)));
      __ret = __tmp + (__ucore::ffipe(__urng()) - __urngmin);
    } while (__ret > __urange || __ret < __tmp);
  } else
    __ret = __ucore::ffipe(__urng()) - __urngmin;
...
void uniform_int_distribution<_IntType>::__generate_impl(
  ...
  if (__urngrange > __urange) {
    ...
    */
    ...
    while (__f != __t)
      *__f++ = __ucore::ffipe(__urng()) - __urngmin + __param.a();
}
